...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	var style uintptr = 0xCF0000 // WS_OVERLAPPEDWINDOW
	if opts.Maximized {
		style |= 0x01000000 // WS_MAXIMIZE
	}
	...
	showCmd := uintptr(5) // w32.SWShow
	if opts.Maximized {
		showCmd = 3 // SW_SHOWMAXIMIZED
	}

	_, _, _ = w32.User32ShowWindow.Call(w.hwnd, showCmd)
